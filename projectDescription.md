...
2. `Organization` (name, slug, contact\_email)

3. `Project` (organization-dependent with name, status, description, due\_date)

4. `Task` (project-dependent with title, description, status, assignee\_email)

5. `TaskComment` (linking to tasks with content, author\_email, timestamp)
...
contact\_email = models.EmailField()
...
status = models.CharField(max\_length=20, choices=STATUS\_CHOICES)
...
project = models.ForeignKey(Project, on\_delete=models.CASCADE)

title = models.CharField(max\_length=200)

description = models.TextField(blank=True)

status = models.CharField(max\_length=20, choices=TASK\_STATUS\_CHOICES)

assignee\_email = models.EmailField(blank=True)

due\_date = models.DateTimeField(null=True, blank=True)
...
status: 'ACTIVE' | 'COMPLETED' | 'ON\_HOLD';
...
9. ✅ Proper database migrations

10. ✅ Mock external integrations
